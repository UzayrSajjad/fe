...
const PlatformBadge: React.FC<{ platform: Platform }> = ({ platform }) => {
  ...
  return (
    <div
      className={`absolute -bottom-0.5 -right-0.5 w-5 h-5 rounded-full ${colors.iconBg} flex items-center justify-center border-2 border-white dark:border-gray-800`}
    >
      {platform === "instagram" && <InstagramIcon className="w-3 h-3 text-white" />}
      {platform === "facebook" && <FacebookIcon className="w-3 h-3 text-white" />}
    </div>
  );
};
              ...
              <div className="relative">
                <div
                  className={`w-12 h-12 rounded-full overflow-hidden ring-2 transition-all ${
                    isSelected
                      ? "ring-brand-500"
                      : "ring-gray-200 dark:ring-gray-700 group-hover:ring-gray-300 dark:group-hover:ring-gray-600"
                  }`}
                >
                  <img
